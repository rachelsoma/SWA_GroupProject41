...
Users were then clustered and visualised in two-dimensional vector space.
...
Clusters 1,3,4 and 5 all appear to be fan accounts, while cluster 2 seems more likely to be news and entertainment websites.
...
##19 out of 26 tweets in cluster 1 are retweets.
retweetsId2=which(tweets[clusterTweetsId2,]$is_retweet==TRUE)
##471 out of 698 Tweets in cluster 2 are retweets.
retweetsId3=which(tweets[clusterTweetsId3,]$is_retweet==TRUE)
##25 out of 50 Tweets in cluster 3 are retweets.
retweetsId4=which(tweets[clusterTweetsId4,]$is_retweet==TRUE)
##34 out of 47 Tweets in cluster 4 are retweets.
retweetsId5=which(tweets[clusterTweetsId5,]$is_retweet==TRUE)
##41 out of 61 tweets in cluster 5 are retweets.
```

2.      Construct a 2xM table where M is the number of user clusters you found at Section 2. Each row (2 rows in total) should represent the total number of retweeted tweets and non-retweeted tweets in each cluster.
```{r}
...
We tested if retweeting is independent of user groups by performing a Chi-Square test for Independence on the data seen in comparison table above
...
```
We Do not reject null nypothesis as pvalue > alpha(0.05)
