...
##Code in line 57-63 is used to create the SWA project.csv file,
## which used for the work to have a consistent set of tweets for analysis.
...
tweets=search_tweets(q="J.K.Rowling",n=1000,lang="en")
1
...
corpus = tm_map(corpus, removeWords,c(stopwords(),"jk","https", "t.co","rowl","jkrowl","will","like","just","doe","far","quot","look","take","make","jaqkcpmtg","ygorfremo","ask","peopl","next","twitter","peopl","new","write","charact","got","ohhgmljqrm","say","defend","respond"))
...
##wordcloud(names(freqs),freqs,scale=c(2,0.5),random.order = FALSE,use.r.layout=FALSE, colors=brewer.pal(8, "Dark2"),min.freq = 3)
...
##Complete Linkage 
h.complete = hclust(D)
h.complete <- color_branches(h.complete, k = 4)
plot(h.complete,main = "Complete")

##Single Linkage
h.single = hclust(D, method="single")
h.single <- color_branches(h.single, k = 4)
plot(h.single,main = "Single")

```


  <h4>ii.    Which one do you think performed better?</h4>
  
  Complete Linkage is the better option as it
...
## After inspecting the plot we are selecting the number of clusters= 4  using the elbow method.
